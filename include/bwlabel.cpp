...
void qrcode::labelNum(Eigen::MatrixXi & label, int & n, std::vector<int>& num)
{
	// number of labels
	n = 0;
	...
	// label index and number of this label
...
void qrcode::labelNum(Eigen::MatrixXi & label, std::vector<int>& num)
{
	// number of labels
	int n = 0;
	for (int i = 0; i < label.rows(); i++) {
...
void qrcode::bwlabel(Engine * engine, Eigen::MatrixXi & bw, int connectivity, Eigen::MatrixXi & label, std::vector<int>& num)
{
	qrcode::bwlabel(engine, bw, connectivity, label);
	...
	//n+1 label(the existence of 0), every number of labels: num
	//int n;
	//std::vector<int> num;
	//qrcode::labelNum(label, n, num);
...
//void qrcode::bwlabel(Engine * engine, Eigen::MatrixXi & bw, int connectivity, Eigen::MatrixXi & label, int label_num)
//{
//	assert(!(connectivity != 4 && connectivity != 8) && "connectivity must be 4 or 8!!!");
//
//	Eigen::MatrixXf BW = bw.cast<float>();
...
//	igl::matlab::mlsetmatrix(&engine, "bw", BW);
//	if (connectivity == 8)
//		igl::matlab::mleval(&engine, "[label, num] = bwlabel(bw,8);");
//	else if (connectivity == 4)
//		igl::matlab::mleval(&engine, "[label, num] = bwlabel(bw,4);");
//
//	Eigen::MatrixXf LABEL;
//	igl::matlab::mlgetmatrix(&engine, "label", LABEL);
//	label = LABEL.cast<int>();
//	Eigen::MatrixXi NUM;
//	NUM.resize(1, 1);
//	igl::matlab::mlgetmatrix(&engine, "num", NUM);
//	label_num = NUM(0,0);
//	
//}
